...
class Radium_Theme_Importer {
	...
	public function demo_installer() {
		...
		if ($action === '') {
			?>
			<div class="wrap postbox uncode-wrap" style="padding: 30px;">
				<div class="inside">
					<div id="import-area">
						<h2><?php echo esc_html__('Install Demo','uncode'); ?></h2>

						<div style="background-color: #F5FAFD; margin:50px 0; padding: 25px 35px; clear:both;">
							<p><?php echo esc_html__('Here you can install demo from our demo site, this is the easiest way to setup your theme. Before you install demo please read the following.','uncode'); ?></p>

							<p><b><?php echo esc_html__('What happens:','uncode'); ?></b></p>
							<ul style="padding-left: 0px; list-style-position: inside; list-style-type: disc;">
								<li><?php echo esc_html__('No existing contents or any other data will be deleted or modified.','uncode'); ?></li>
								<li><?php echo esc_html__('No WordPress settings will be modified.','uncode'); ?></li>
								<li><?php echo esc_html__('Posts, pages, images, widgets and menus will get imported.','uncode'); ?></li>
								<li><?php echo esc_html__('Please click import only once and wait, it can take few minutes.','uncode'); ?></li>
							</ul>

							<p><b><?php echo esc_html__('Important notes:','uncode'); ?></b></p>
							<ul style="padding-left: 0px; list-style-position: inside; list-style-type: disc;">
								<li><?php printf(esc_html__('Make sure you don\'t have any red message in your %s.','uncode'), '<a href="' . esc_url(admin_url('admin.php?page=uncode-menu') ) . '">'.esc_html__('System Status','uncode').'</a>'); ?></li>
								<li><?php printf(esc_html__('Deactivate extra plugins except the theme\'s %s.','uncode'), '<a href="' . esc_url('//support.undsgn.com/hc/en-us/articles/213454589') . '" target="_blank">'.esc_html__('official plugins','uncode').'</a>'); ?></li>
								<li><?php echo esc_html__('It\'s discouraged to import demo contents on your existing site.','uncode'); ?></li>
								<li><?php printf(esc_html__('The will import an exact replica of the demo site with %s.','uncode'), '<a href="' . esc_url('//support.undsgn.com/hc/en-us/articles/214001065#demo-images') . '" target="_blank">'.esc_html__('placeholder images','uncode').'</a>'); ?></li>
							</ul>
						</div>
						<?php
							...
							if($response && $can_read_file) {
								...
								<form class="js-one-click-import-form" style="text-align: center;">
									<input class="panel-save button-primary" type="submit" value="<?php echo esc_html__('INSTALL DEMO','uncode'); ?>" style="height: 60px; width: 250px; font-size: 18px;" />
								</form>
								<?php
							} else { ?>
								<div>
					...
					?>
						<hr style="border-top-width: 1px; margin: 50px 0 40px" />
			...
			<script type="text/javascript">
				jQuery( function ( $ ) {
					...
					$('.js-one-click-import-form' ).on( 'submit', function (e) {
						...
						menu_import = '<?php echo wp_kses_post( __('IMPORTANT\n\n1. This action will not import the Uncode demo site main menu. If you want to import the menu of the demo site as well, please use "Import Menu" button on this page.\n\n2. If you are importing demos to an existing installation of Uncode note that this action will also overwrite your Theme Options. If you need to import some specific layouts to an existing installation of Uncode, please use "Single Layouts" button.', 'uncode') ) . '\n'; ?>',
						...
						var c = ($(e.currentTarget).hasClass('import-ot') || $(e.currentTarget).hasClass('import-widgets') || $(e.currentTarget).hasClass('import-menu')) ? confirm("<?php esc_html_e('This will replace your settings, are you sure?', 'uncode'); ?>") : confirm(inactive_plugins + ucore_active + cf7_active + woo_active + "\n" + menu_import + "\n<?php echo esc_html__('Are you sure?', 'uncode'); ?>");
						if (c) {
					...
					function uncode_import_demo(data) {
						...
						$.ajax({
							...
							success: function(response, textStatus, xhr) {
								...
								if (!$(is_fine).length) {
									if ($(response).find('.post-imported').length > 0 && runned < 20) {
										...
										result = '<?php printf(esc_html__('Ooops, the Demo Content couldn\'t be imported all in once. Please refer to this %s for a possible workaround.','uncode'), '<a href="' . esc_url('support.undsgn.com/hc/en-us/articles/213459629') . '" target="_blank">'.esc_html__('troubleshoot thread','uncode').'</a>'); ?>';
							...
							error: function (xhr, ajaxOptions, thrownError) {
								thrownError = '<?php printf(esc_html__('Ooops, the Demo Content couldn\'t be imported all in once. Please refer to this %s for a possible workaround.','uncode'), '<a href="' . esc_url('support.undsgn.com/hc/en-us/articles/213459629') . '" target="_blank">'.esc_html__('troubleshoot thread','uncode').'</a>'); ?>';
								if (runned < 10) {
									runned++;
									uncode_import_demo(this_data);
								} else {
